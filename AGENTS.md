...
- **🔴 ОТКРЫТАЯ ЗАДАЧА (новая): Аномалия среднего курса по расходам после распределения**
  - *Симптом:* средний курс перевода по расходам ≈ 34 (аномально), по доходам ≈ 22 (норма). Примерный курс ≈ 20-25. По отдельным позициям расчётный курс достигает сотен/тысяч/миллионов.
.
  - *Гипотеза (пользователя):* расхождение возникает при **распределении расходов** (по номенклатуре и контрагентам, шаги 15/16): при распределении некоторые доли стремятся к `0`, из-за округлений перевод в рубли становится некорректным.
.
  - *Направления решения:* (а) увеличить число знаков после запятой при промежуточных расчётах распределения (`оборот_распределенный_руб`, `доля_ном_группы`); (б) финальное округление выполнять уже на финальном отчёте (`step_19`/`step_13`), а не на промежуточных шагах; (в) проверить точность при `groupby().sum()` (`float64`), не теряются ли значащие цифры кори. .
  - *Где смотреть:* `pipeline/steps/base_expenses_step.py` (распределение в `_process_accumulation_transactions`: `df_cross['оборот_распределенный_руб']`, доли `доля_ном_группы`; `_create_remainder_rows`); `step_17` (groupby 91.01/91.02); `step_18` (99.
.  - *Как воспроизвести:* AED-компания; расчётный курс по позициям расходов = `оборот, тыс.руб.` / `оборот, тыс.ед.` — аномально отличается от курса листа на дату баланса.
