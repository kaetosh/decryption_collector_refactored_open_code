...
#     # Признаки продажи активов
#     # ASSET_SALE_TYPES = ['Продажа прочего имущества', 'Продажа основных средств', 'Аренда']
...
#         # создаём 'сегмент' СРАЗУ, пока df_9101 ещё не стал view
#         segment_company = refs['companies'].loc[
#             refs['companies']['сокращенное_наименование_компании'] == name_company,
#             'сегмент'
#         ].iloc[0]
...
#         # ★ Диагностика сразу после создания
#         logger.debug(
...
#         # ★ НОВОЕ: Извлечение контрагентов для ОБЕИХ таблиц
#         # ★ Извлечение контрагентов — передаём список счетов из справочника
#         df_9101 = self._extract_contractors(
...
#         # 5. Обработка ППА (только для 91.01)
#         # 5. Обработка ППА (91.01 и 91.02)
#         # df_9101 = self._process_ppa(df_9101, refs['ppa'])
#         df_9101, df_9102 = self._process_ppa(df_9101, df_9102, refs['ppa'])
...
#         # 7. Обработка кредитных линий (РБП)
#         df_9101 = self._process_credit_lines(df_9101, refs['credit'], is_income=True)
#         df_9102 = self._process_credit_lines(df_9102, refs['credit'], is_income=False)
...
#         # 8. Добавление служебных столбцов
#         df_9101 = self._add_service_columns(df_9101, self.ACCOUNT_OTHER_INCOME)
#         df_9102 = self._add_service_columns(df_9102, self.ACCOUNT_OTHER_EXPENSE)

        
#         # 10. Объединение с main_df
#         df_final = self._merge_with_main_df(context.journal_df, df_9101, df_9102)
...
#     def _filter_91_transactions(
#         self, transactions_all_df: pd.DataFrame, reference_df: pd.DataFrame
#     ) -> tuple[pd.DataFrame, pd.DataFrame]:
#         """Фильтрует проводки по 91.01 и 91.02, подтягивает вид_дохода_расхода."""
#         logger.debug("Фильтрация проводок 91.01 и 91.02")
...
#         # ★ НОВОЕ: Подтягиваем вид_дохода_расхода из справочника Меппинг_опу
#         df_9101 = self._add_income_expense_type(df_9101, reference_df, is_income=True)
#         df_9102 = self._add_income_expense_type(df_9102, reference_df, is_income=False)
...
#     def _add_income_expense_type(
#         self, df: pd.DataFrame, reference_df: pd.DataFrame, is_income: bool
#     ) -> pd.DataFrame:
#         """Подтягивает вид_дохода_расхода из справочника Меппинг_опу."""
#         account_label = self.ACCOUNT_OTHER_INCOME if is_income else self.ACCOUNT_OTHER_EXPENSE
    
#         # ★ Переименование: для 91.01 Кт = наш счёт (91.01), Дт = корр.счёт
#         #                   для 91.02 Дт = наш счёт (91.02), Кт = корр.счёт
#         account_col = 'Кт' if is_income else 'Дт'
#         corr_col = 'Дт' if is_income else 'Кт'
#         df = df.rename(columns={account_col: 'счет', corr_col: 'Корр.счет'})
    
#         # Для 91.01 доход_расход = Субконто Кт_1
#         # Для 91.02 доход_расход = Субконто Дт_1
#         subconto_col = 'Субконто Кт_1' if is_income else 'Субконто Дт_1'
#         df = df.rename(columns={subconto_col: 'доход_расход'})
...
#         # ★ БЕЗ fillna — оставляем NaN для проверки
#         df['вид_дохода_расхода'] = df['_key'].map(mapping).astype('string')
...
#     # def _process_9101(
#     #     self, df_9101: pd.DataFrame, reference_ppa_df: pd.DataFrame
#     # ) -> pd.DataFrame:
#     #     """Обрабатывает прочие доходы: извлекает контрагентов, обрабатывает ППА."""
#     #     logger.debug("Обработка 91.01: контрагенты и ППА")

#     #     # Обработка ППА
#     #     df_9101 = self._process_ppa(df_9101, reference_ppa_df)

#     #     return df_9101
    
#     def _process_ppa(
#         self, df_9101: pd.DataFrame, df_9102: pd.DataFrame, reference_ppa_df: pd.DataFrame
#     ) -> pd.DataFrame:
#         """Подтягивает контрагентов из справочника ППА."""
#         logger.debug("Обработка ППА")
        
#         df_9101 = df_9101.copy()  # ★ защита от view
#         df_9102 = df_9102.copy()  # ★ защита от view

#         PPA_INCOME_TYPE_9101 = "Доходы от выбытия прав пользования активами, изменения условий договоров аренды"
#         PPA_INCOME_TYPE_9102 = "Расходы от выбытия прав пользования активами, изменений условий договоров аренды"
#         mask_ppa_income_9101 = df_9101['вид_дохода_расхода'] == PPA_INCOME_TYPE_9101
#         mask_ppa_income_9102 = df_9102['вид_дохода_расхода'] == PPA_INCOME_TYPE_9102
...
#         # Добавляем столбец с объектом ППА
#         mask_ppa_accounts_9101 = df_9101['Корр.счет'].astype(str).str.startswith(self.PPA_ACCOUNTS, na=False)
#         mask_ppa_accounts_9102 = df_9102['Корр.счет'].astype(str).str.startswith(self.PPA_ACCOUNTS, na=False)
        
#         df_9101['объект для изм ппа'] = df_9101['Субконто Дт_1'].where(
#             mask_ppa_income_9101 & mask_ppa_accounts_9101, 'не_указано'
#         ).astype('string')
        
#         df_9102['объект для изм ппа'] = df_9102['Субконто Кт_1'].where(
#             mask_ppa_income_9102 & mask_ppa_accounts_9102, 'не_указано'
#         ).astype('string')
...
#         # Условие 1: Корр.счет начинается с 01.09 → поиск по ос_ппа
#         mask_01_09_9101 = mask_ppa_income_9101 & df_9101['Корр.счет'].astype(str).str.startswith('01.09', na=False)
#         mapped_1_9101 = df_9101['объект для изм ппа'].map(mapping_ppa)
#         df_9101['контрагент'] = np.where(
#             mask_01_09_9101 & mapped_1_9101.notna(),
#             mapped_1_9101,
...
#         # Условие 1: Корр.счет начинается с 01.09 → поиск по ос_ппа
#         mask_01_09_9102 = mask_ppa_income_9102 & df_9102['Корр.счет'].astype(str).str.startswith('01.09', na=False)
#         mapped_1_9102 = df_9102['объект для изм ппа'].map(mapping_ppa)
#         df_9102['контрагент'] = np.where(
#             mask_01_09_9102 & mapped_1_9102.notna(),
#             mapped_1_9102,
...
#         # Условие 2: Корр.счет начинается с 02.01 или 01.01 → поиск по ос_после_перехода
#         mask_02_01_or_01_01_9101 = mask_ppa_income_9101 & df_9101['Корр.счет'].astype(str).str.startswith(
#             ('02.01', '01.01'), na=False
#         )
#         mapped_2_9101 = df_9101['объект для изм ппа'].map(mapping_transfer)
#         df_9101['контрагент'] = np.where(
#             mask_02_01_or_01_01_9101 & mapped_2_9101.notna(),
...
#         # Условие 2: Корр.счет начинается с 02.01 или 01.01 → поиск по ос_после_перехода
#         mask_02_01_or_01_01_9102 = mask_ppa_income_9102 & df_9102['Корр.счет'].astype(str).str.startswith(
#             ('02.01', '01.01'), na=False
#         )
#         mapped_2_9102 = df_9102['объект для изм ппа'].map(mapping_transfer)
#         df_9102['контрагент'] = np.where(
...
#         count_ppa_9101 = (mask_01_09_9101 & mapped_1_9101.notna()).sum() + (mask_02_01_or_01_01_9101 & mapped_2_9101.notna()).sum()
#         count_ppa_9102 = (mask_01_09_9102 & mapped_1_9102.notna()).sum() + (mask_02_01_or_01_01_9102 & mapped_2_9102.notna()).sum()
        
#         logger.debug(f"Подтянуто {count_ppa_9101+count_ppa_9102} контрагентов из справочника ППА")
...
#     #     mask_ppa_income = df_9101['вид_дохода_расхода'] == PPA_INCOME_TYPE

#     #     if not mask_ppa_income.any():
#     #         return df_9101

#     #     # Добавляем столбец с объектом ППА
#     #     mask_ppa_accounts = df_9101['Корр.счет'].astype(str).str.startswith(self.PPA_ACCOUNTS, na=False)
#     #     df_9101['объект для изм ппа'] = df_9101['Субконто Дт_1'].where(
#     #         mask_ppa_income & mask_ppa_accounts, 'не_указано'
#     #     ).astype('string')
...
#     #     # Условие 1: Корр.счет начинается с 01.09 → поиск по ос_ппа
#     #     mask_01_09 = mask_ppa_income & df_9101['Корр.счет'].astype(str).str.startswith('01.09', na=False)
#     #     mapped_1 = df_9101['объект для изм ппа'].map(mapping_ppa)
#     #     df_9101['контрагент'] = np.where(
#     #         mask_01_09 & mapped_1.notna(),
#     #         mapped_1,
#     #         df_9101['контрагент']
#     #     )

#     #     # Условие 2: Корр.счет начинается с 02.01 или 01.01 → поиск по ос_после_перехода
#     #     mask_02_01_or_01_01 = mask_ppa_income & df_9101['Корр.счет'].astype(str).str.startswith(
#     #         ('02.01', '01.01'), na=False
#     #     )
#     #     mapped_2 = df_9101['объект для изм ппа'].map(mapping_transfer)
#     #     df_9101['контрагент'] = np.where(
#     #         mask_02_01_or_01_01 & mapped_2.notna(),
#     #         mapped_2,
#     #         df_9101['контрагент']
#     #     )
...
#     #     count_ppa = (mask_01_09 & mapped_1.notna()).sum() + (mask_02_01_or_01_01 & mapped_2.notna()).sum()
#     #     logger.debug(f"Подтянуто {count_ppa} контрагентов из справочника ППА")
...
#         df_9101: pd.DataFrame,
#         df_9102: pd.DataFrame,
#         asset_sale_types: list  # ★ НОВОЕ: принимаем как параметр
#     ) -> tuple[pd.DataFrame, pd.DataFrame]:
#         """Обрабатывает продажу активов: НДС, контрагенты, распределение."""
#         logger.debug("Обработка продажи активов")
...
#         # ★ Используем параметр вместо self.ASSET_SALE_TYPES
#         mask_9101_assets = df_9101['вид_дохода_расхода'].isin(asset_sale_types)
#         mask_9102_assets = df_9102['вид_дохода_расхода'].isin(asset_sale_types)
...
#             f"({mask_9101_assets.sum()} строк в 91.01, {mask_9102_assets.sum()} строк в 91.02)"
#         )
        
#         # --- Задача 1: Корректировка выручки на НДС ---
#         df_9101, df_9102 = self._adjust_revenue_for_vat(
#             df_9101, df_9102, mask_9101_assets, mask_9102_assets
#         )
...
#         df_9101: pd.DataFrame,
#         df_9102: pd.DataFrame,
#         mask_9101_assets: pd.Series,
#         mask_9102_assets: pd.Series
#     ) -> tuple[pd.DataFrame, pd.DataFrame]:  # ← ВОЗВРАЩАЕМ ОБА!
#         """Уменьшает выручку от реализации активов на сумму НДС."""
#         logger.debug("Корректировка выручки на НДС")
        
#         # Выделяем НДС из df_9102
#         df_9102_assets = df_9102[mask_9102_assets]
#         mask_vat = df_9102_assets['Корр.счет'].astype(str).str.startswith(self.NDS_ACCOUNTS, na=False)
#         df_9102_vat = df_9102_assets[mask_vat]
        
#         if df_9102_vat.empty:
#             logger.debug("НДС по продаже активов не обнаружен")
#             return df_9101, df_9102  # ← оба
...
#         # Применяем к df_9101
#         df_9101_assets = df_9101[mask_9101_assets].copy()
#         df_9101_assets = df_9101_assets.merge(vat_by_doc, on='Документ', how='left')
#         df_9101_assets['ндс_тыс_ед'] = df_9101_assets['ндс_тыс_ед'].fillna(0)
        
#         # ★ ИСПРАВЛЕНИЕ: Доходы со знаком МИНУС, поэтому для уменьшения
#         # по модулю СКЛАДЫВАЕМ с положительным НДС
#         # Пример: было -1000, НДС=+160 → стало -1000 + 160 = -840
#         df_9101_assets['оборот, тыс.ед.'] = (
#             df_9101_assets['оборот, тыс.ед.'] + df_9101_assets['ндс_тыс_ед']
#         )
        
#         # Обновляем df_9101
#         df_9101.loc[mask_9101_assets, 'оборот, тыс.ед.'] = df_9101_assets['оборот, тыс.ед.'].values
        
#         # ★ ИСПРАВЛЕНИЕ: удаляем строки НДС из df_9102 и ВОЗВРАЩАЕМ
#         mask_vat_in_main = mask_9102_assets & df_9102['Корр.счет'].astype(str).str.startswith(self.NDS_ACCOUNTS, na=False)
#         df_9102 = df_9102[~mask_vat_in_main].copy()
        
#         adjusted = (df_9101_assets['ндс_тыс_ед'] > 0).sum()
#         removed_vat = mask_vat_in_main.sum()
#         logger.debug(
#             f"Скорректировано {adjusted} строк выручки на НДС, "
#             f"удалено {removed_vat} строк НДС из 91.02"
#         )
        
#         return df_9101, df_9102  # ← оба датафрейма
...
#         # Маска: строки продажи активов с 'не_указано' в контрагенте
#         # mask_unspecified = mask_9102_assets & (df_9102['контрагент'] == 'не_указано')
#         mask_unspecified = (
#                 mask_9102_assets.fillna(False).astype(bool) & 
#                 (df_9102['контрагент'] == 'не_указано').fillna(False).astype(bool)
#             )
...
#         # Подтягиваем контрагента
#         mapped_contractors = df_9102.loc[mask_unspecified, 'Документ'].map(contractor_mapping)
#         df_9102.loc[mask_unspecified, 'контрагент'] = np.where(
#             mapped_contractors.notna(),
#             mapped_contractors,
#             'не_указано'
#         )
...
#         if connection_mapping is not None and 'вид_связи' in df_9102.columns:
#             mapped_connections = df_9102.loc[mask_unspecified, 'Документ'].map(connection_mapping)
#             df_9102.loc[mask_unspecified, 'вид_связи'] = np.where(
#                 mapped_connections.notna(),
#                 mapped_connections,
#                 df_9102.loc[mask_unspecified, 'вид_связи']
#             )
...
#         replaced = mapped_contractors.notna().sum()
#         logger.debug(f"Подтянуто {replaced} контрагентов из 91.01 в 91.02")
...
#         mask_orphan = mask_9102_assets & df_9102['Документ'].isin(orphan_docs)
        
#         df_9102_orphan = df_9102[mask_orphan].copy()
#         # df_9102_orphan = df_9102[mask_orphan.to_numpy()].copy()
        
#         df_9102_attached = df_9102[~mask_orphan].copy()
#         # df_9102_attached = df_9102[~mask_orphan.to_numpy()].copy()

#         # Расчёт долей выручки по контрагентам в рамках вид_дохода_расхода
#         df_9101_assets = df_9101[mask_9101_assets].copy()
#         revenue_by_type = df_9101_assets.groupby('вид_дохода_расхода')['оборот, тыс.ед.'].transform('sum')
#         df_9101_assets['доля_контрагента'] = df_9101_assets['оборот, тыс.ед.'] / revenue_by_type

#         contractor_share = (
#             df_9101_assets.groupby(['вид_дохода_расхода', 'контрагент'], as_index=False)
#             ['доля_контрагента'].sum()
#         )
...
#         # Удаляем контрагент перед merge (чтобы не было суффиксов _x/_y)
#         df_9102_orphan_clean = df_9102_orphan.drop(columns=['контрагент'], errors='ignore')
...
#         if df_9102_orphan_dist.empty:
#             logger.warning("⚠️ Не удалось распределить осиротевшие расходы (нет выручки)")
#             return df_9102

#         # Распределяем оборот
#         df_9102_orphan_dist['оборот, тыс.ед.'] = (
#             df_9102_orphan_dist['оборот, тыс.ед.'] * df_9102_orphan_dist['доля_контрагента']
#         )
#         df_9102_orphan_dist = df_9102_orphan_dist.drop(columns=['доля_контрагента'])
...
#         # Собираем обратно
#         df_9102_result = pd.concat([df_9102_attached, df_9102_orphan_dist], ignore_index=True)
...
#     def _process_credit_lines(
#         self, df: pd.DataFrame, reference_rbp_credit: pd.DataFrame, is_income: bool = True
#     ) -> pd.DataFrame:
#         """Обрабатывает кредитные линии: подтягивает контрагентов из справочника."""
#         CREDIT_TYPE = 'Кредитное обслуживание и расходы по открытию кредитных линий'
#         mask_credit = (
...
#         df['рбп_кредитные_линии'] = df[subconto_col].where(mask_credit, 'не_указано').astype('string')
...
#         df['контрагент'] = df['контрагент'].astype('string')
#         logger.debug(f"Кредитные линии: {mask_found.sum()} контрагентов подтянуто")
...
#         logger.debug(f"вид_связи: {df['вид_связи'].value_counts().to_dict()}")
        
#         return df

#     def _calculate_connection_type(
#         self, df: pd.DataFrame, segment_company: str
#     ) -> pd.Series:
#         """
#         Рассчитывает вид_связи на основе группа_ка и сегмент_ка.
...
#     def _add_service_columns(self, df: pd.DataFrame, account: str) -> pd.DataFrame:
#         """Добавляет служебные столбцы для соответствия структуре main_df."""
#         # ★ Столбец 'счет' уже создан в _add_income_expense_type
#         # Просто проверяем и приводим к string
#         if 'счет' not in df.columns:
#             df['счет'] = account
#             df['счет'] = df['счет'].astype('string')
#         else:
...
#         # Убираем технические столбцы
#         cols_to_drop = [
#             'Имя_файла', 'Дата', 'Сумма', 'Содержание_1', 'Содержание_2',
#             'Субконто Дт_1', 'Субконто Дт_2', 'Субконто Дт_3',
#             'Субконто Кт_1', 'Субконто Кт_2', 'Субконто Кт_3',
#             'Дт', 'Кт', 'Корр.счет',
#         ]
...
#         # ★ ГРУППИРОВКА по указанным столбцам
#         group_cols = [
#             'счет', 'вид_дохода_расхода', 'доход_расход', 'контрагент',
#             'объект для изм ппа', 'группа_ка', 'сегмент_ка', 'сегмент',
#             'вид_связи', 'рбп_кредитные_линии', 'рбп_проценты'
#         ]
#         # Оставляем только существующие столбцы
#         existing_group_cols = [col for col in group_cols if col in df_combined.columns]
...
#         df_combined = df_combined.groupby(
#             existing_group_cols, as_index=False
#         )['оборот, тыс.ед.'].sum()
        
#         logger.debug(
#             f"Группировка: {len(df_9101) + len(df_9102)} → {len(df_combined)} строк"
#         )
...
#             'объект для изм ппа', 'рбп_кредитные_линии', 'рбп_проценты'
#         ]
#         for col in text_cols:
#             if col in df_final.columns:
#                 df_final[col] = df_final[col].astype('string')
...
#     def _extract_contractors(
#         self, df: pd.DataFrame, is_income: bool, accounts_with_contractors: tuple
#     ) -> pd.DataFrame:
#         """
#         Извлекает контрагентов из Субконто.
